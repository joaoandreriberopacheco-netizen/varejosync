...
export default function AgefinConsulta() {
  ...
  const imprimirRelatorio = async () => {
    ...
    const cabecalhoColunasHtml = `<div style="display:grid;grid-template-columns:96px 96px 520px 120px;column-gap:10px;align-items:end;margin:8px 0 10px;padding:0 10px;color:#475569;border-bottom:1px solid #cfd8e3;padding-bottom:6px"><div style="font-size:12px;line-height:1.25;font-weight:700">Vencimento</div><div style="font-size:12px;line-height:1.25;font-weight:700">Status</div><div style="font-size:12px;line-height:1.25;font-weight:700">Conta</div><div style="font-size:12px;line-height:1.25;font-weight:700;text-align:right">Valor</div></div>`;
    ...
    const gruposHtml = gruposParaImpressao.map((grupo) => {
      ...
      const linhas = grupo.contas.map((conta) => {
        ...
        return `<div style="padding:8px 10px;border-bottom:1px solid #dde5ef;background:#ffffff">
          <div style="display:grid;grid-template-columns:96px 96px 520px 120px;column-gap:10px;row-gap:3px;align-items:start">
            <div style="grid-row:1 / span 2;font-size:12px;line-height:1.3;color:#334155;padding-top:1px">${escapeHtml(formatarSoData(conta.data_vencimento))}</div>
            <div style="grid-row:1 / span 2;display:flex;align-items:center;padding-top:0">
              <span style="display:inline-flex;align-items:center;gap:4px;padding:2px 7px;border-radius:999px;background:${statusBg};border:1px solid #d8e1ec;color:${statusColor};font-size:11px;line-height:1.2;font-weight:700;white-space:nowrap"><span style="font-size:12px;line-height:1">${statusIcon}</span>${statusLabel}</span>
            </div>
            <div style="min-width:0;font-size:13.2px;line-height:1.3;font-weight:700;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(conta.descricao || '-')}</div>
            <div style="grid-row:1 / span 2;text-align:right;padding-top:1px;font-size:13.2px;line-height:1.3;font-weight:700;color:#0f172a">${escapeHtml(formatCurrency(conta.valor))}</div>
            <div style="min-width:0;font-size:11.8px;line-height:1.25;color:#64748b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(conta.terceiro_nome || 'Sem favorecido')}${conta.categoria ? ` · ${escapeHtml(conta.categoria)}` : ''}</div>
          </div>
        </div>`;
      }).join('');

      return `<section style="margin-top:12px;border:1px solid #d5dde8;border-radius:8px;overflow:hidden;background:#f1f3f5"><div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:7px 10px;background:#eceff3;border-bottom:1px solid #d5dde8"><div style="font-size:12px;line-height:1.25;font-weight:700;color:#334155">Vencimento ${escapeHtml(grupo.label)}</div><div style="text-align:right"><span style="font-size:11.4px;line-height:1.25;color:#64748b">${grupo.contas.length} · </span><span style="font-size:12.2px;line-height:1.25;font-weight:700;color:#0f172a">${escapeHtml(formatCurrency(subtotal))}</span></div></div>${linhas}</section>`;
    }).join('');

    const html = `<html><head><meta charset="UTF-8" /><title>Agefin ${escapeHtml(formatMonth(currentMonth))}</title></head><body style="font-family:'Noto Sans','NotoSans',Arial,sans-serif;padding:18px;color:#111827;font-size:12px;line-height:1.3"><div style="width:872px;min-width:872px;max-width:872px"><div style="background:#f8fafc;border:1px solid #d5dde8;border-radius:8px;padding:8px 10px;margin-bottom:8px"><h2 style="margin:0 0 2px;font-size:18px;line-height:1.1;color:#0f172a">Agefin - ${escapeHtml(formatMonth(currentMonth))}</h2><p style="margin:0 0 2px;color:#64748b;font-size:12px;line-height:1.2">Contas filtradas da consulta financeira</p><p style="margin:0 0 2px;color:#64748b;font-size:12px;line-height:1.2">Quantidade: ${contasParaImpressao.length} conta${contasParaImpressao.length !== 1 ? 's' : ''}</p><p style="margin:0;color:#64748b;font-size:12px;line-height:1.2">Total impresso: <span style="font-weight:700;color:#0f172a">${escapeHtml(formatCurrency(totalParaImpressao))}</span></p>${modoSelecao ? `<p style="margin:2px 0 0;color:#64748b;font-size:12px;line-height:1.2">Modo Somar: apenas contas selecionadas</p>` : ''}</div>${filtrosHtml}${cabecalhoColunasHtml}${gruposHtml}</div></body></html>`;
    try {
