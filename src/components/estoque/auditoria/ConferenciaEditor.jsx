...
// Operário vê somente nome do produto, SEM ver estoque do sistema (contagem cega)
export default function ConferenciaEditor({ conferencia: conferenciaInicial, onVoltar }) {
  ...
  const isBloqueada = ["Aguardando Auditoria", "Concluída", "Cancelada"].includes(conferenciaInicial.status);
  ...
  useEffect(() => {
    ...
    const filtrados = produtos
      ...
      .sort((a, b) => (a.nome || a.campo_hierarquico_1 || "").localeCompare(b.nome || b.campo_hierarquico_1 || "", "pt-BR"))
      .slice(0, 30);
  ...
  const atualizarQtd = async (idx, delta) => {
    if (isBloqueada) return;
    const novosItens = itens.map((item, i) => i !== idx ? item : { ...item, quantidade_contada: Math.max(0, (item.quantidade_contada || 0) + delta) });
    setItens(novosItens);
    await salvarItens(novosItens);
  };
  ...
  // Operário finaliza → vai para "Aguardando Auditoria" (responsável vai verificar)
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500">
            {itens.length} entrada{itens.length !== 1 ? "s" : ""} · {itensAgrupados.length} produto{itensAgrupados.length !== 1 ? "s" : ""}
          </p>
        </div>
        {isBloqueada && (
          <div className="flex items-center gap-1 text-xs text-orange-500 bg-orange-50 dark:bg-orange-900/20 px-2 py-1 rounded-lg flex-shrink-0">
      ...
      {/* Banner de bloqueio */}
      {isBloqueada && (
        <div className="mb-4 p-3 rounded-xl bg-orange-50 dark:bg-orange-900/20 text-xs text-orange-600 dark:text-orange-400 flex items-center gap-2">
          <Lock className="w-4 h-4 flex-shrink-0" />
          <span>Esta conferência está <strong>{conferenciaInicial.status}</strong> e não pode ser editada. Aguarde o responsável fazer a auditoria.</span>
        </div>
      )}

      {/* Busca — só exibe se não bloqueado */}
      {!isBloqueada && (
            ...
            <div className="absolute top-full left-0 right-0 z-30 mt-1 bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden max-h-72 overflow-y-auto">
              <div className="divide-y divide-gray-50 dark:divide-gray-700">
                {produtosFiltrados.map(prod => {
                  ...
                  return (
                    <button key={prod.id} onClick={() => selecionarProduto(prod)}
              ...
              <div className="border-t border-gray-100 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
                {grupo.entradas.map((entrada, eIdx) => (
                  <div key={entrada.idx} className="flex items-center gap-3 px-3.5 py-2.5">
                    <span className="text-xs text-gray-400 dark:text-gray-500 w-16">Entrada {eIdx + 1}</span>
                    <div className="flex items-center gap-2 flex-1 justify-end">
                      ...
                      <Input
                        type="number" inputMode="numeric" value={entrada.qtd}
                        onChange={e => definirQtd(entrada.idx, e.target.value)}
                        readOnly={isBloqueada}
      ...
      {/* Botão finalizar — só aparece se não bloqueado */}
      {!isBloqueada && (
        <div className="border-t border-gray-100 dark:border-gray-800 pt-4">
          <Button
            onClick={finalizar}
            disabled={finalizando || itens.length === 0}
            className="w-full h-12 rounded-xl bg-gray-900 dark:bg-white text-white dark:text-gray-900 shadow-none text-sm font-medium"
          >
            {finalizando
          ...
          <p className="text-center text-[10px] text-gray-400 dark:text-gray-500 mt-2">
            Ao enviar, a contagem será bloqueada e o responsável irá auditar
          </p>
        </div>
      )}
      ...
      {modalQtd && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 pb-4 sm:pb-0">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setModalQtd(null)} />
          <div className="relative bg-white dark:bg-gray-900 rounded-3xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex items-center gap-3 mb-6">
              <button onClick={() => setModalQtd(m => ({ ...m, qtdStr: String(Math.max(0, (parseFloat(m.qtdStr) || 0) - 1)) }))}
              ...
              <Input ref={qtdInputRef} type="number" inputMode="numeric" value={modalQtd.qtdStr}
                onChange={e => setModalQtd(m => ({ ...m, qtdStr: e.target.value }))}
                onKeyDown={e => e.key === "Enter" && confirmarQtd()}
                className="flex-1 text-center text-2xl font-bold font-glacial border-0 bg-gray-50 dark:bg-gray-800 rounded-xl h-14 focus-visible:ring-1 focus-visible:ring-gray-200 dark:focus-visible:ring-gray-700"
              />
              <button onClick={() => setModalQtd(m => ({ ...m, qtdStr: String((parseFloat(m.qtdStr) || 0) + 1) }))}
                className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
