      ...
      <DialogContent className="max-w-lg rounded-[28px] border-0 bg-white p-0 shadow-2xl dark:bg-background">
        ...
        <div className="space-y-4 p-4 md:p-5">
          <div>
            ...
            <Input value={valor} onChange={handleValorChange} className="h-11 rounded-2xl border-0 bg-gray-100 shadow-sm dark:bg-muted" />
          </div>
          <div>
            <Label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Observação</Label>
            <Textarea value={observacao} onChange={(e) => setObservacao(e.target.value)} className="min-h-[96px] rounded-[24px] border-0 bg-gray-100 shadow-sm dark:bg-muted" />
          </div>
          <div>
            <Label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Motivo do ajuste</Label>
            <Textarea value={motivo} onChange={(e) => setMotivo(e.target.value)} className="min-h-[88px] rounded-[24px] border-0 bg-gray-100 shadow-sm dark:bg-muted" />
          </div>

          <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
            <Button onClick={() => onEdit({ valor, observacao, motivo })} disabled={!motivo.trim()} className="h-11 rounded-2xl bg-gray-900 text-white hover:bg-primary dark:bg-white dark:text-foreground">
