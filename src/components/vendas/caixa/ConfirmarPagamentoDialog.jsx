...
import { Banknote, Smartphone, CreditCard, Receipt, Edit, CheckCircle2, RefreshCw, FileText, Terminal } from 'lucide-react';
...
export default function ConfirmarPagamentoDialog({
  ...
  codigoVale, setCodigoVale, valeEncontrado, setValeEncontrado,
  buscandoVale, setBuscandoVale,
  pagamentosContaPagar, setPagamentosContaPagar, inputContaPagar, setInputContaPagar,
  // maquininha state - passados do PDVCaixa
  ...
  const [showSeletorDebito, setShowSeletorDebito] = useState(false);
  const [showSeletorCredito, setShowSeletorCredito] = useState(false);
  ...
  const handleDebitoFocus = (e) => {
    e.target.select();
    setFormaPagamentoAtiva(2);
    // Abre seletor se não há maquininha ainda
    if (!maquininhaDebito) setShowSeletorDebito(true);
  };

  const handleCreditoFocus = (e) => {
    e.target.select();
    setFormaPagamentoAtiva(3);
    if (!maquininhaCredito) setShowSeletorCredito(true);
  };
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto dark:bg-gray-900 dark:text-gray-200">
          ...
          {pedidoSelecionado && (
            <div className="space-y-4">
                    ...
                    <div key={idx} className="px-3 py-2 border-b border-gray-100 dark:border-gray-800 last:border-0">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-400">{item.quantidade} × R$ {item.preco_unitario_praticado?.toFixed(2)}</div>
                        </div>
                        <div className="text-sm font-semibold text-gray-800 dark:text-gray-200">R$ {item.total?.toFixed(2)}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Formas de Pagamento */}
              <div className="space-y-4">
                <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Formas de Pagamento</p>
                <div className="space-y-1">

                  {/* Dinheiro */}
                  <div className={`flex items-center justify-between py-4 cursor-pointer rounded-lg transition-colors ${formaPagamentoAtiva === 0 ? 'bg-gray-50 dark:bg-gray-800 -mx-2 px-2' : ''}`}
                    onClick={() => { setFormaPagamentoAtiva(0); inputRefs.dinheiro.current?.focus(); }}>
                    <div className="flex items-center gap-3">
                      <Banknote className="w-5 h-5 text-gray-400" />
                      <span className="text-base text-gray-700 dark:text-gray-300">Dinheiro</span>
                    </div>
                    <input ref={inputRefs.dinheiro} type="text" inputMode="numeric" value={inputDinheiro} onChange={() => {}}
                      onKeyDown={(e) => handleInputMascara(e, setInputDinheiro, setPagamentosDinheiro)}
                      onFocus={(e) => { e.target.select(); setFormaPagamentoAtiva(0); }}
                  ...
                  {/* PIX */}
                  <div className={`flex items-center justify-between py-4 cursor-pointer rounded-lg transition-colors ${formaPagamentoAtiva === 1 ? 'bg-gray-50 dark:bg-gray-800 -mx-2 px-2' : ''}`}
                    onClick={() => { setFormaPagamentoAtiva(1); inputRefs.pix.current?.focus(); }}>
                    <div className="flex items-center gap-3">
                      <Smartphone className="w-5 h-5 text-gray-400" />
                      <span className="text-base text-gray-700 dark:text-gray-300">PIX</span>
                    </div>
                    <input ref={inputRefs.pix} type="text" inputMode="numeric" value={inputPix} onChange={() => {}}
                      onKeyDown={(e) => handleInputMascara(e, setInputPix, setPagamentosPix)}
                      onFocus={(e) => { e.target.select(); setFormaPagamentoAtiva(1); }}
                  ...
                  {/* Cartão Débito */}
                  <div className={`rounded-lg transition-colors ${formaPagamentoAtiva === 2 ? 'bg-gray-50 dark:bg-gray-800 -mx-2 px-2' : ''}`}>
                    <div className="flex items-center justify-between py-4 cursor-pointer"
                      onClick={() => { setFormaPagamentoAtiva(2); inputRefs.debito.current?.focus(); }}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {maquininhaDebito && (
                            <p className="text-xs text-gray-400 dark:text-gray-500">
                              {maquininhaDebito.maquininha.nome} · {maquininhaDebito.bandeira} · {maquininhaDebito.taxa}%
                              <button onClick={e => { e.stopPropagation(); setMaquininhaDebito(null); setShowSeletorDebito(true); }} className="ml-2 text-blue-500 underline">trocar</button>
                            </p>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {pagamentosDebito > 0 && !maquininhaDebito && (
                          <button onClick={e => { e.stopPropagation(); setShowSeletorDebito(true); }} className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-lg">
                            <Terminal className="w-3 h-3 inline mr-1" />Escolher maquininha
                          </button>
                        )}
                        <input ref={inputRefs.debito} type="text" inputMode="numeric" value={inputDebito} onChange={() => {}}
                          onKeyDown={(e) => handleInputMascara(e, setInputDebito, setPagamentosDebito)}
                          onFocus={handleDebitoFocus}
                  ...
                  {/* Cartão Crédito */}
                  <div className={`rounded-lg transition-colors ${formaPagamentoAtiva === 3 ? 'bg-gray-50 dark:bg-gray-800 -mx-2 px-2' : ''}`}>
                    <div className="flex items-center justify-between py-4 cursor-pointer"
                      onClick={() => { setFormaPagamentoAtiva(3); inputRefs.credito.current?.focus(); }}>
                      <div className="flex items-center gap-3">
                        <CreditCard className="w-5 h-5 text-gray-400" />
                        <div>
                          ...
                          {maquininhaCredito && (
                            <p className="text-xs text-gray-400 dark:text-gray-500">
                              {maquininhaCredito.maquininha.nome} · {maquininhaCredito.bandeira} · {maquininhaCredito.taxa}%
                              <button onClick={e => { e.stopPropagation(); setMaquininhaCredito(null); setShowSeletorCredito(true); }} className="ml-2 text-blue-500 underline">trocar</button>
                            </p>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {pagamentosCredito > 0 && !maquininhaCredito && (
                          <button onClick={e => { e.stopPropagation(); setShowSeletorCredito(true); }} className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-lg">
                            <Terminal className="w-3 h-3 inline mr-1" />Escolher maquininha
                          </button>
                        )}
                        <input ref={inputRefs.credito} type="text" inputMode="numeric" value={inputCredito} onChange={() => {}}
                          onKeyDown={(e) => handleInputMascara(e, setInputCredito, setPagamentosCredito)}
                          onFocus={handleCreditoFocus}
                        ...
                        {pagamentosCredito > 0 && (
                          <Select value={parcelasCredito.toString()} onValueChange={(v) => setParcelasCredito(parseInt(v))}>
                            ...
                            <SelectContent className="dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200">
                              {[...Array(12)].map((_, i) => (
                                <SelectItem key={i + 1} value={(i + 1).toString()} className="dark:hover:bg-gray-700">{i + 1}x</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        )}
                  ...
                  {/* Conta a Pagar (Fiado) */}
                  <div className={`flex items-center justify-between py-4 cursor-pointer rounded-lg transition-colors ${formaPagamentoAtiva === 5 ? 'bg-gray-50 dark:bg-gray-800 -mx-2 px-2' : ''}`}
                    onClick={() => { setFormaPagamentoAtiva(5); inputRefs.contaPagar?.current?.focus(); }}>
                    <div className="flex items-center gap-3">
                    ...
                    <input ref={inputRefs.contaPagar} type="text" inputMode="numeric" value={inputContaPagar} onChange={() => {}}
                      onKeyDown={(e) => handleInputMascara(e, setInputContaPagar, setPagamentosContaPagar)}
                      onFocus={(e) => { e.target.select(); setFormaPagamentoAtiva(5); }}
                  ...
                  <div className={`py-4 rounded-lg transition-colors ${formaPagamentoAtiva === 4 ? 'bg-gray-50 dark:bg-gray-800 -mx-2 px-2' : ''}`}>
                    <div className="flex items-center justify-between cursor-pointer" onClick={() => setFormaPagamentoAtiva(4)}>
                      ...
                      <input ref={inputRefs.vale} type="text" inputMode="numeric" value={inputVale} onChange={() => {}}
                        onKeyDown={(e) => {
                          if (valeEncontrado) {
                    ...
                    <div className="mt-2 flex gap-2">
                      <input type="text" placeholder="Código do vale (ex: VC-00001)"
                        value={codigoVale} onChange={e => setCodigoVale(e.target.value.toUpperCase())}
                        onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleBuscarVale(); } }}
                      ...
                      <button onClick={handleBuscarVale} className="h-9 px-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-lg text-sm" disabled={buscandoVale}>
                        {buscandoVale ? '...' : 'Buscar'}
                      </button>
                    </div>
                    {valeEncontrado && (
                ...
                <div className="pt-4 mt-4 border-t border-gray-100 dark:border-gray-700 space-y-2">
                  ...
                  {pagamentoValido && <p className="text-sm text-center text-gray-400 pt-3">↵ Enter para aprovar</p>}
                </div>
              </div>

              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setShowRetornoDialog(true)} className="flex-1 h-12 gap-2 border-gray-300 dark:border-gray-600">
                  <Edit className="w-4 h-4" />Retornar para Edição
                </Button>
                <Button onClick={handleFinalizarVenda} disabled={!pagamentoValido || processandoVenda}
      ...
      {/* Seletor de maquininha - Débito */}
      <SeletorMaquininhaSheet
        visible={showSeletorDebito}
        modalidade="debito"
        parcelas={1}
        onSelect={(dados) => { setMaquininhaDebito(dados); setShowSeletorDebito(false); }}
        onCancel={() => setShowSeletorDebito(false)}
      />

      {/* Seletor de maquininha - Crédito */}
      <SeletorMaquininhaSheet
        visible={showSeletorCredito}
        modalidade="credito"
        parcelas={parcelasCredito}
        onSelect={(dados) => { setMaquininhaCredito(dados); setShowSeletorCredito(false); }}
        onCancel={() => setShowSeletorCredito(false)}
      />
    </>
  );
}