...
export default function SimuladorCartaoSheet({ open, onClose, valorTotal, valorDesconto }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className="relative w-full md:max-w-md bg-white dark:bg-gray-900 rounded-t-3xl md:rounded-3xl shadow-2xl z-10 overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Venda de R$ {fmt(valorTotal)}</p>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="px-5 pb-6 space-y-4 overflow-y-auto flex-1">
            ...
            <div className="flex rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 p-0.5 gap-0.5">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 h-9 rounded-lg text-xs font-medium transition-all ${
                  !modoJurosCliente ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400'
                }`}
              >
            ...
            <div className="bg-gray-50 dark:bg-gray-800/60 rounded-2xl p-4 space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-xs text-gray-400">Valor cobrado ao cliente</span>
                <span className="text-sm font-semibold text-gray-900 dark:text-white">R$ {fmt(valorTotal)}</span>
              </div>
              ...
              {modoJurosCliente ? (
                ...
                <>
                  <div className="space-y-1.5 border-t border-gray-100 dark:border-gray-700 pt-2">
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-gray-400">Empresa paga (intermediação)</span>
                      <span className="text-xs text-red-500">- R$ {fmt(valorTotal * (taxa_interm_vendor_modo_cliente / 100))}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-gray-400">Juros cobrado do cliente ({taxaJurosClienteMensal}%/mês)</span>
                      <span className="text-xs text-gray-400">suportado pelo cliente</span>
                    </div>
                    {parcelaModoCliente && modalidade === 'Crédito Parcelado' && (
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-gray-400">Parcela do cliente ({parcelas}x)</span>
                        <span className="text-xs font-medium text-gray-700 dark:text-gray-300">R$ {fmt(parcelaModoCliente)}</span>
                      </div>
                    )}
                  </div>
                  <div className="flex justify-between items-center border-t border-gray-100 dark:border-gray-700 pt-2">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-gray-400">{prazoStr()}</div>
                    </div>
                  </div>
                </>
              ) : (
                ...
                <>
                  <div className="space-y-1.5 border-t border-gray-100 dark:border-gray-700 pt-2">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400">- R$ {fmt(valorTotal * taxa_intermediacao / 100)}</span>
                    </div>
                    {taxa_parcelamento > 0 && (
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-gray-400">Parcelamento ({taxa_parcelamento.toFixed(2)}%)</span>
                        <span className="text-xs text-gray-500 dark:text-gray-400">- R$ {fmt(valorTotal * taxa_parcelamento / 100)}</span>
                      </div>
                    )}
                  ...
                  <div className="flex justify-between items-center border-t border-gray-100 dark:border-gray-700 pt-2">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Você receberá</span>
                    <div className="text-right">
                      <div className="text-xl font-bold text-gray-900 dark:text-white">R$ {fmt(valor_liquido)}</div>
                      <div className="text-xs text-gray-400">{prazoStr()}</div>
                    </div>
                  </div>
                </>
