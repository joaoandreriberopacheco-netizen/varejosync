...
export default function PullToRefreshWrapper({ onRefresh, children, className = '' }) {
  ...
  return (
    <div ref={containerRef} className={`relative overflow-auto ${className}`}>
      ...
      >
        <div className={`flex items-center gap-2 bg-white dark:bg-gray-800 shadow-sm rounded-full px-3 py-1.5 text-xs text-gray-500 dark:text-gray-400 ${isRefreshing ? '' : ''}`}>
          <RefreshCw className={`w-3.5 h-3.5 ${isRefreshing ? 'animate-spin text-emerald-500' : 'text-gray-400'}`} />
