...
export default function DetalhesPedidoCompra({ pedido, isOpen, onClose }) {
  ...
  const loadDadosAdicionais = async () => {
    try {
      ...
      // Buscar Supermanifesto
      if (pedido.supermanifesto_id) {
        const manifesto = await base44.entities.Supermanifesto.filter({ id: pedido.supermanifesto_id });
        if (manifesto && manifesto.length > 0) {
          setSupermanifesto(manifesto[0]);
        }
      }
    } catch (error) {
        ...
        <DialogContent className="bg-white p-6 z-50 grid w-full !max-w-[95vw] !w-[95vw] gap-4 border shadow-lg duration-200 sm:rounded-lg dark:bg-gray-900 max-h-[95vh] overflow-y-auto">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-blue-700 dark:text-blue-300">
                    Vincule este pedido a um manifesto de transporte
                  </p>
                </div>
                <Button
          ...
          {supermanifesto && (
            <div className="p-4 bg-teal-50 dark:bg-teal-900/20 rounded-lg border border-teal-200 dark:border-teal-800">
              <h4 className="text-sm font-semibold text-teal-900 dark:text-teal-200 mb-2 flex items-center gap-2">
                <Ship className="w-4 h-4" />
