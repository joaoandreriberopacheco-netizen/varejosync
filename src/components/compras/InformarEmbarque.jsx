    ...
    <>
      ...
      <div className="fixed inset-0 z-[401] flex items-center justify-center p-4" onClick={e => e.stopPropagation()}>
        <div className="w-full max-w-lg max-h-[92vh] flex flex-col bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden">
                  ...
                  {(pedido.itens || []).map(item => {
                    ...
                    return (
                      <div key={item.produto_id}
                        className={`flex items-start gap-2 rounded-xl bg-white dark:bg-gray-700/50 px-3 py-2.5 transition-opacity ${!selecionado ? 'opacity-40' : ''}`}>
                        <button
                          type="button"
                          onClick={() => toggleItem(item.produto_id, pedida)}
                          className={`w-5 h-5 mt-0.5 rounded flex-shrink-0 border flex items-center justify-center transition-colors ${
                            selecionado
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                            Ped: {fmtNum(pedida)} {item.unidade_medida}
                            {outrosEmb > 0 && <span className="ml-1 text-teal-500">· já emb: {fmtNum(outrosEmb)}</span>}
                            {disponivel < pedida && disponivel > 0 && <span className="ml-1 text-gray-500">· disp: {fmtNum(disponivel)}</span>}
                            {excede && <span className="ml-1 text-rose-500">· excede!</span>}
                          </p>
                        </div>
                        <div className="flex-shrink-0 flex flex-col items-end gap-0.5">
                          <Input
                            ...
                            className={`w-20 h-9 text-sm text-right border-0 rounded-xl bg-gray-50 dark:bg-gray-700 shadow-sm text-gray-900 dark:text-gray-100 px-2 ${excede ? 'ring-1 ring-rose-400' : ''}`}
                          />
          ...
          {/* Footer */}
          <div className="flex justify-end gap-3 px-5 py-5 border-t border-gray-100 dark:border-gray-800 flex-shrink-0">
            <Button variant="outline" onClick={onClose} disabled={loading}
              className="border-0 shadow-sm h-12 px-6 rounded-xl text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700">
              Cancelar
            </Button>
            <Button
              type="button"
              ...
              disabled={loading}
              className={`${isEdicao ? 'bg-gray-800 hover:bg-gray-900 dark:bg-white dark:text-gray-900' : 'bg-teal-600 hover:bg-teal-700'} text-white border-0 shadow-sm h-12 px-6 rounded-xl min-w-[170px] disabled:pointer-events-none disabled:opacity-100`}
            >
              {loading
