...
import { Truck, Package, Calendar, AlertTriangle, CheckCircle2, ChevronDown, ChevronUp, Edit3, Boxes } from 'lucide-react';
...
export default function InformarEmbarque({ pedido, isOpen, onClose, onSuccess, embarqueExistente }) {
  ...
  const [volumes, setVolumes] = useState([]); // array de objetos
  const [observacoes, setObservacoes] = useState('');
  const [loading, setLoading] = useState(false);
  const [showItens, setShowItens] = useState(true);
  const [qtdEmbarque, setQtdEmbarque] = useState({});
  const [itensSelecionados, setItensSelecionados] = useState({}); // { [produto_id]: bool }
  ...
  const toggleItem = (produtoId, pedida) => {
    ...
    if (!novoSel) {
      ...
      const outrosEmb = jaEmbarcadoSemEste[produtoId] || 0;
  ...
  // Resumo de volumes para exibição inline
  const volumesResumo = useMemo(() => {
    ...
    return `${totalVol} volumes · ${total.toFixed(1)} kg`;
  }, [volumes]);
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-900 border-0 shadow-2xl">
          <DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input type="datetime-local" value={eta} onChange={e => setEta(e.target.value)}
                className="bg-gray-50 dark:bg-gray-800 border-0 shadow-sm text-gray-900 dark:text-gray-100" />
            </div>
              ...
              {showItens && (
                <div className="rounded-xl bg-gray-50 dark:bg-gray-800/60 p-2 space-y-1.5">
                  {(pedido.itens || []).map(item => {
                    ...
                    return (
                      <div key={item.produto_id} className={`flex items-center gap-2 rounded-lg px-2 py-1.5 transition-colors ${!selecionado ? 'opacity-40' : ''}`}>
                        {/* Checkbox manual */}
                        <button
                          type="button"
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-gray-400 dark:text-gray-500">
                            ...
                            {disponivel < pedida && disponivel > 0 && <span className="ml-1 text-amber-500">· Disp: {disponivel}</span>}
                          </p>
                        </div>
                        <div className="w-20">
                          <Input
