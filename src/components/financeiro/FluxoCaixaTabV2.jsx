...
function LancRow({ l, onClick }) {
  ...
  return (
    <button
      onClick={() => onClick(l)} className="flex flex-col gap-2 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors text-left bg-white dark:bg-gray-600/50">
...
export default function FluxoCaixaTabV2() {
  ...
  const kpis = useMemo(() => {
    ...
    filtrados.forEach((l) => {
      if (l.status === 'Pago') {if (l.tipo === 'Receita') entrou += l.valor || 0;else if (l.tipo === 'Despesa') saiu += l.valor || 0;} else
      {if (l.tipo === 'Receita') pEntrou += l.valor || 0;else if (l.tipo === 'Despesa') pSaiu += l.valor || 0;}
    });
