...
export default function AgefinPortal() {
  ...
  const stats = useMemo(() => {
    ...
    const hoje = new Date();
    const vencidas = contasNoPeriodo.filter((c) => new Date(`${c.data_vencimento}T12:00:00`) < hoje);
    ...
    const comBoleto = contasNoPeriodo.filter((c) => c.boleto_url || c.tem_boleto).length;
  ...
  }, [contas, periodoInicio, periodoFim]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        {/* Contas com boleto */}
        <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-3xl p-4 shadow-sm border-l-4 border-emerald-400">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <p className="text-xs font-semibold text-emerald-700 dark:text-emerald-300 mb-1">Com boleto</p>
              <p className="text-3xl font-bold text-emerald-900 dark:text-emerald-100">{stats.comBoleto}</p>
              <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-2">contas prontas para consulta</p>
            </div>
            <DollarSign className="w-10 h-10 text-emerald-300 dark:text-emerald-700 opacity-40" />
          </div>
        </div>
      </div>
