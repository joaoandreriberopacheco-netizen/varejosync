  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-3xl p-5 shadow-sm space-y-3">
      <div>
        ...
        <h3 className="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100 font-glacial">{evento.transportadora_nome || evento.embarcacao_nome}</h3>
      ...
      <div className="rounded-2xl bg-gray-50 dark:bg-gray-700 p-3 shadow-sm">
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400"><CalendarRange className="w-4 h-4" /> Cabeçalho da viagem</div>
        <div className="mt-2 text-sm text-gray-900 dark:text-gray-100 flex items-center gap-2"><ShipWheel className="w-4 h-4" /> ETA: {evento.data_chegada_destino_formatada || '-'}</div>
        <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Código: {evento.codigo || '-'}</div>
