...
function getContaStatusStyle(temConta, status, estaAtrasada) {
  if (!temConta) return { bgClass: 'bg-muted/50', strokeColor: '#d1d5db', label: 'Sem vinculação' };
    ...
    <div className="space-y-4 pb-4">
      ...
      <div className="rounded-3xl bg-card p-4 shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-foreground">{evento.embarcacao_nome}</p>
      ...
      {temConta && contaAtualizada ? (
        <div className="rounded-3xl bg-muted/50 p-4 space-y-2 text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-foreground">{statusConta}</span>
          </div>
          {contaAtualizada.data_vencimento && (
            <div className="flex justify-between">
              <span className="text-muted-foreground">Vencimento:</span>
              <span className={`font-semibold ${estaAtrasada ? 'text-red-600 dark:text-red-400' : 'text-foreground'}`}>
      ...
      <div className="rounded-3xl bg-muted/50 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-muted-foreground">Valor total do frete</span>
          </div>
      ...
      <div className="rounded-3xl bg-muted/50 p-4 space-y-2">
        <p className="text-xs font-semibold text-foreground mb-2">Embarques vinculados</p>
        <div className="text-xs text-muted-foreground space-y-1">
          <div className="flex justify-between">
            <span>Total de embarques:</span>
            <span className="font-medium text-foreground">{embarques?.length || 0}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium text-foreground">{evento.total_fornecedores_relacionados || 0}</span>
          </div>
      ...
      {!contaAtualizada?.id && (
        <div className="rounded-3xl bg-muted/50 p-4 text-center text-xs text-muted-foreground">
