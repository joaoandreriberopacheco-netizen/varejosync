...
export default function CentralAcoes() {
  ...
  return (
    <div className="pb-6 mb-6 border-b border-border/40">
      {/* Header */}
      <h2 className="text-sm font-medium text-foreground mb-4">Ações Pendentes</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {modulos.map((modulo) => {
          ...
          return (
            <button 
              key={modulo.nome}
              className="p-4 bg-muted/50 rounded-lg hover:bg-muted transition-all text-center group shadow-sm border border-transparent dark:border-border/40"
            >
              <Icon className="w-6 h-6 mx-auto text-muted-foreground dark:text-foreground mb-2 transition-colors" />
              <div className="text-2xl font-bold text-foreground mb-1 tracking-tight">{modulo.count}</div>
              <div className="text-xs text-muted-foreground font-medium uppercase tracking-wide">{modulo.nome}</div>
            </button>
          );
        })}
      </div>
