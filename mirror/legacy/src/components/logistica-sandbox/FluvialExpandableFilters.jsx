          ...
          <div className="rounded-[28px] bg-[#111827]/95 dark:bg-[#111827]/95 shadow-2xl backdrop-blur-sm overflow-hidden">
            <div className="flex items-center justify-between gap-3 p-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-2xl bg-card/10 flex items-center justify-center shadow-sm">
              ...
              <Button
                ...
                className="h-10 w-10 rounded-2xl bg-card/10 text-white hover:bg-card/15"
              >
            ...
            <CollapsibleContent className="px-3 pb-3 data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
              <div className="space-y-3 max-h-[62vh] overflow-y-auto pr-1">
                <div className="rounded-3xl bg-card/5 p-3 shadow-sm space-y-2">
                  <button
                    type="button"
                    onClick={() => onOnlyLinkedChange?.(!onlyLinked)}
                    className={`w-full flex items-center justify-between gap-3 rounded-2xl text-sm shadow-sm px-3 py-2.5 ${onlyLinked ? 'bg-card text-foreground' : 'bg-card/10 text-white'}`}
                  >
                  ...
                  {onlyLinked ? (
                    <div className="grid grid-cols-3 gap-2">
                      <button type="button" onClick={() => onLinkedStatusChange?.('todos')} className={`rounded-2xl px-3 py-2 text-xs shadow-sm ${linkedStatus === 'todos' ? 'bg-card text-foreground' : 'bg-card/10 text-white'}`}>Todos</button>
                      <button type="button" onClick={() => onLinkedStatusChange?.('ativos')} className={`rounded-2xl px-3 py-2 text-xs shadow-sm ${linkedStatus === 'ativos' ? 'bg-lime-300 text-foreground' : 'bg-card/10 text-white'}`}>Ativos</button>
                      <button type="button" onClick={() => onLinkedStatusChange?.('concluidos')} className={`rounded-2xl px-3 py-2 text-xs shadow-sm ${linkedStatus === 'concluidos' ? 'bg-muted text-foreground' : 'bg-card/10 text-white'}`}>Concluídos</button>
                    </div>
                  ) : null}
                </div>
